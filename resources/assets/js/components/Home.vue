<template>
    <div>
        ...
        <side-bar :pluckCategories="pluckCategories"  :sideBar="toggleSide" v-model="categories"></side-bar>
        <div zclass="w3-main" :style="toggleSide ? toggleSidebar_open : toggleSidebar_close ">
            ...
            <div :class="{'w3-row-padding w3-theme':layout == 'grid',  'w3-content': layout == 'list'}">
                <product v-for="product in selectedFilteredCategories"
                         :product="product" :class="{'w3-third w3-section':layout == 'grid' }"
                         :layout="layout">
                </product>
            </div>
...
<script>
    ...
    export default{
        ...
        beforeDestroy(){
            eventBus.$off('toggleState', this.toggleSide)
        },
        computed: {
            selectedFilteredCategories() {
              var selectCategories;
              var categories = this.categories;
              var selectedFilter = _.filter(this.filteredData, function (select) { return select.category ? select.category.name ? categories.indexOf(select.category.name) != -1 : select.category.name : null });

              if(_.isEmpty(categories)){
                  selectCategories = this.filteredData
              }else {
                  selectCategories = selectedFilter
              }
              return selectCategories

            },
            pluckCategories(){
                var categories = this.filteredData
                var map = _.map(categories, function (num, key) { return num.category ? num.category.name : null });
